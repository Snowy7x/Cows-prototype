...
namespace SimpleFolderIcon.Editor
{
    ...
    {
        private const string AssetsPath = "_Core/Plugins/SimpleFolderIcon/Icons";
        ...
        internal static void BuildDictionary()
        {
            ...
            {
                var texture = (Texture)AssetDatabase.LoadAssetAtPath($"Assets/_Core/Plugins/SimpleFolderIcon/Icons/{f.Name}", typeof(Texture2D));
                dictionary.Add(Path.GetFileNameWithoutExtension(f.Name),texture);
            }
            dir = new DirectoryInfo(Application.dataPath + "/" + AssetsPath + "/Presets");
            FileInfo[] infoSO = dir.GetFiles("*.asset");

            foreach (FileInfo f in infoSO) 
            {
                var folderIconSO = (FolderIconSO)AssetDatabase.LoadAssetAtPath($"Assets/_Core/Plugins/SimpleFolderIcon/Icons/Presets/{f.Name}", typeof(FolderIconSO));
